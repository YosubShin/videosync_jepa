...
def main(args_eval, resume_preempt=False):
    ...
    folder = os.path.join(pretrain_folder, 'video_sync_frozen/')
...
def run_one_epoch(
    ...
    for itr, data in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_bfloat16):
            ...
            imgs, labels, fname = data[0].to(
                device), data[1].to(device), data[2]
            with torch.no_grad():
                outputs = encoder(imgs)

                logger.info(
                    f'itr: {itr}, outputs.shape: {outputs.shape}, fname: {fname}')

                batch_size = outputs.size(0)
                num_frames = 1
                tokens_per_frame = int((224/16) * (224/16))
                # assert outputs.size(1) == num_frames * tokens_per_frame
                embedding_dim = outputs.size(2)
                reshaped_tensor = outputs.view(
                    batch_size, num_frames * tokens_per_frame * embedding_dim)

                for i in range(batch_size):
                    single_frame_tensor = reshaped_tensor[i]
                    assert single_frame_tensor.size(
                        0) == num_frames * tokens_per_frame * embedding_dim

                    embeds[fname[i]] = single_frame_tensor

    torch.save(embeds, '/home/yosubs/koa_scratch/embeds.pth')
